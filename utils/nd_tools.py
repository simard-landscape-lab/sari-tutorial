...
def get_superpixel_means_band(label_array, band):
    # Assume labels are 0, 1, 2, ..., n
...
def get_superpixel_stds_band(label_array, band):
    # Assume labels are 0, 1, 2, ..., n
    labels_ = label_array + 1  # scipy wants labels to begin at 1 and transforms to 1, 2, ..., n+1
    labels_unique = np.unique(labels_)
...
def lee_filter_band(band, size, mask=None):
...
def scale_img(img: np.ndarray,
              ...
              new_max=1):
    """
    Scale an image by the absolute max and min in the array.
    """
    i_min = np.nanmin(img)
    i_max = np.nanmax(img)
    if i_min == i_max:
        # then image is constant image and round to 0 or 1
